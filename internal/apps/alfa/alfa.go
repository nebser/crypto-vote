...
func Initialize(blockchain _blockchain.Blockchain, options Options) (_blockchain.Blockchain, error) {
	...
	if err != nil {
		return _blockchain.Blockchain{}, errors.Wrap(err, "Failed to generate genesis transaction")
	}
	genesisBlock, err := _blockchain.NewBlock(nil, transaction.Transactions{*genesisTransaction})
	if err != nil {
		return _blockchain.Blockchain{}, errors.Wrap(err, "Failed to create genesis block")
	}
	initialized, err := blockchain.SetGenesis(*genesisBlock)
	if err != nil {
		return _blockchain.Blockchain{}, errors.Wrap(err, "Failed to initialize blockchain")
	}
	keyfiles, err := getKeyFiles(options.ClientKeysDir)
	if err != nil {
		return _blockchain.Blockchain{}, errors.Wrapf(err, "Failed to retrieve key files from directory %s", options.ClientKeysDir)
	}
	wallets, err := loadWallets(keyfiles)
	if err != nil {
		return _blockchain.Blockchain{}, errors.Wrap(err, "Failed to load wallets")
	}
	baseTransactions := transaction.Transactions{}
	for _, w := range wallets {
		t, err := transaction.NewBaseTransaction(*masterWallet, w.Address)
		if err != nil {
			return _blockchain.Blockchain{}, errors.Wrapf(err, "Failed to create transaction to wallet %#v", w)
		}
	...
	return initialized.AddBlock(*block)
